...
const SenimanPage = () => {
  ...
  useEffect(() => {
    fetch("http://127.0.0.1:5000/api/users")
      ...
      .catch((err) => console.error("Gagal mengambil data seniman:", err));
  }, []);

  useEffect(() => {
    const hasil = senimanList.filter((s) => `${s.nama_lengkap} ${s.username} ${s.bio || ""}`.toLowerCase().includes(query.toLowerCase()));
    setFiltered(hasil);
  }, [query, senimanList]);
          ...
          {filtered.map((user, i) => (
            ...
            >
              <div className="relative w-fit mx-auto mb-4">
                <Avatar src={user.foto_profil} size={80} icon={<UserOutlined />} className="bg-purple-500 text-white text-xl border-4 border-white shadow-md">
                  {!user.foto_profil && user.nama_lengkap?.charAt(0)?.toUpperCase()}
                </Avatar>
              </div>

              <h3 className="text-xl font-semibold text-gray-800 group-hover:text-purple-700 transition">{user.nama_lengkap || "Tanpa Nama"}</h3>
