...
const RuangVideo = () => {
  ...
  useEffect(() => {
    ...
    const likedArray = Array.isArray(rawLiked) ? rawLiked : Object.keys(rawLiked || {});
    setLiked(likedArray);
  }, []);
  ...
  const handleLike = (id) => {
    ...
    const updated = videos.map((vid) => {
      if (vid.id === id) {
        return {
          ...vid,
          likes: isLiked ? Math.max((vid.likes || 1) - 1, 0) : (vid.likes || 0) + 1,
        };
      }
      return vid;
    });
    ...
    setVideos(updated);
    setLiked(newLiked);
    localStorage.setItem("videoList", JSON.stringify(updated));
    localStorage.setItem("likedVideos", JSON.stringify(newLiked));
  };

  const filtered = videos.filter((vid) => (vid.title?.toLowerCase() || "").includes(query.toLowerCase()) || (vid.creator?.toLowerCase() || "").includes(query.toLowerCase()));
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    Oleh: <span className="font-medium text-gray-800 ">{vid.creator || "Tidak diketahui"}</span>
                  </p>

                  <p className="text-sm text-gray-500 mt-2 line-clamp-2">{vid.description}</p>

                  <div onClick={() => handleLike(vid.id)} className="mt-3 flex items-center gap-1 cursor-pointer select-none w-fit hover:scale-105 transition">
